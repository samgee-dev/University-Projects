...
class GridCrafterView(tk.Frame):
    ...
    def __init__(self, master, input_size):
        ...
        # ...
        # Create a frame that will have the crafting packed into it.  Create the button and input/output widgets using
        # the ItemGridView function.
        self._crafting_frame = tk.Frame(master)
        self._crafting_input = ItemGridView(self._crafting_frame, input_size)
        self._crafting_output = ItemGridView(self._crafting_frame, (1, 1))
        self._crafting_button = tk.Button(self._crafting_frame, text="=> Craft =>")
        self._crafting_frame.pack(side=tk.TOP)
        self._crafting_input.pack(side=tk.LEFT)
        self._crafting_button.pack(side=tk.LEFT)
        self._crafting_output.pack(side=tk.LEFT)
    ...
    def render(self, key_stack_pairs, selected):
        ...
        # Task 2.2 Crafting: Create widgets here
        # ...
        # Draw or render the crafting boxes in
        ...
        for key, stack in key_stack_pairs:
            ...
            if key == "output":
                ...
                # ...
                self._crafting_output.draw_cell((0, 0), stack)
            else:
                # Task 2.2 Crafting: Draw input cells
                # ...
                self._crafting_input.draw_cell(key, stack)
    ...
    def bind_for_id(self, event, callback):
        ...
        # Allow the crafting boxes and button to be interacted with.
        self._crafting_input.bind(event, lambda e: callback(self.xy_to_grid((e.x, e.y)), e))
        self._crafting_output.bind(event, lambda e: callback('output', e))
        self._crafting_button.bind(event, lambda e: callback('craft', e))

    # The below code is from another python file which is responsible for determining what box is being interacted with
    # so it has been used again here
