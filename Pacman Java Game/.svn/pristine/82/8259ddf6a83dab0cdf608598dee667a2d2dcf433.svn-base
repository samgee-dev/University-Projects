...
public class BoardViewModelTest {
     ...
     * list in the default case.
     */
    @Test
    public void getGhostsDefault() {
        ...
        // Since the getGhost() function can be in any order, a for loop will
        // be used to iterate through each element on the list to check if
        // there is a match. If there is no match, it will fail.
        for (int i = 0; i < 4; i++) {
            Boolean checkIfFail = true;

            for (int j = 0; j < 4; j++) {
                // Compare the order and if it matches, change the value of the
                // boolean to be false
                if (ghostTestList.get(i).equals(this.boardViewTest.getGhosts()
                        .get(j))) {
                    checkIfFail = false;
    ...
    /**
     * Test for getGhosts() function. Check that is is returning the correct
     * list when there are changes made.
     */
    @Test
    public void getGhostsAdvanced() {
        // Change some of the positions and the colour of the ghost
        this.pacmanGameTest.getGhosts().get(0).setPosition(new
                Position(2,2));
        this.pacmanGameTest.getGhosts().get(2).setPosition(new
                Position(3,3));
        this.pacmanGameTest.getGhosts().get(1).setPhase(Phase.FRIGHTENED,
                50);
        this.pacmanGameTest.getGhosts().get(2).setPhase(Phase.FRIGHTENED,
                20);

        // Check that the correct list is being returned
        List<Pair<Position,String>> ghostTestList2 = new
                ArrayList<Pair<Position,String>>();
        ghostTestList2.addAll(
                List.of(new Pair<>(new Position(2,2), "#d54e53"),
                        new Pair<>(new Position(0,0), "#0000FF"),
                        new Pair<>(new Position(3,3), "#0000FF"),
                        new Pair<>(new Position(0,0), "#c397d8")));


        // Iterate through the list and check that the elements are present in
        // both. Else, fail the test
        for (int i = 0; i < 4; i++) {
            Boolean checkIfFail = true;
            ...
            for (int j = 0; j < 4; j++) {
                // Compare the order and if it matches, change the value of the
                // boolean to be false
                if (ghostTestList2.get(i).equals(this.boardViewTest.getGhosts()
                        .get(j))) {
                    checkIfFail = false;
